...
<head>
	...
	<script>
		...
		function test4Check() {
			...
			// form 태그에 액션태그를 사용하지 못할 상황이라면? 자바스크립트에서 action 명령으로 이동할 곳의 경로를 지정해 준다.
...
<div class="container">
	...
	<hr/>
	<div>
		<input type="button" value="test2.jsp" onclick="location.href='${ctp}/mapping/test2';" class="btn btn-success "/>
		<input type="button" value="test3.jsp" onclick="location.href='${ctp}/mapping/test3';" class="btn btn-primary "/>
		<input type="button" value="test4.jsp" onclick="test4Check()" class="btn btn-info "/>
	</div>
	<hr/>
	<div>
		<div>
		<%--<form name="myform" method="post" action="${ctp}/mapping/test4">--%>
			<form name="myform" method="post">
				...
				<input type="number" name="su2" value="${empty su2 ? 0 : su2}" class="form-control mb-2" class="form-control mb-2"/>
			</form>
		</div>
		<div>두수의 합 : ${hap}</div>
		<div>두수의 차 : ${cha}</div>
